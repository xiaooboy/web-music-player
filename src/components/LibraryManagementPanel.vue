...
<template>
  <section class="library-panel library-management-panel">
    <div class="section-head">
      <div>
        <h2>{{ sourceNamesLabel || "还没有添加音乐源" }}</h2>
      </div>
      <span class="library-status">{{ sources.length ? `共 ${sources.length} 个音乐源` : "等待添加" }}</span>
    </div>

    <p class="library-management-hint">{{ libraryHint }}</p>

    <div class="library-management-actions">
      <button class="primary-button library-action-button" type="button" @click="$emit('openFolder')">
        <FolderPlus :size="18" />
        <span>添加音乐源</span>
      </button>
      <button class="ghost-button library-action-button" type="button" @click="$emit('openFallback')">
