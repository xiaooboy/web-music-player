...
const activeAlbum = computed(() => props.albums.find((album) => album.name === props.selectedAlbumName) || null);
</script>
...
<template>
  <section class="library-panel album-browser">
    <div class="section-head">
      <div>
        <h2>{{ selectedAlbumName || "按专辑浏览" }}</h2>
      </div>
      <span class="library-status">{{ albums.length ? `共 ${albums.length} 张专辑` : "等待扫描" }}</span>
    </div>
      ...
      <section v-if="activeAlbum" class="album-detail">
        <div class="album-detail-head">
          <div class="album-detail-cover">
            <img v-if="activeAlbum.coverUrl" :src="activeAlbum.coverUrl" :alt="`${activeAlbum.name} 封面`" />
          ...
          <button class="primary-button album-play-button" type="button" @click="$emit('playAlbum', activeAlbum.name)">
            <Play :size="16" />
            <span>播放专辑</span>
          </button>
        </div>
        ...
        <div class="album-song-list">
          ...
          >
            <div class="album-song-main">
              <div class="album-song-icon">
                <Pause v-if="index === currentTrackIndex && isPlaying" :size="16" />
                <Play v-else :size="16" />
              </div>
            ...
            <span class="album-song-duration">{{ formatTime(track.duration) }}</span>
          </button>
        </div>
      </section>
    </div>
